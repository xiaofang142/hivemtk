...
| G11 | `internal/aiagent/agent/lifecycle/`（`active.go` 新增）+ `internal/app/agent_lifecycle_wiring.go` | **主动触达骨架未落地**（2026-08-25 快照原话）。**（T-P5-02 已让"双模式"第一次有一个入口，2026-09-21 实跑登记）** 开工前实测到的不是"缺一套主动引擎"，而是这四个各自独立的事实：① `lifecycle.go` 只有接口与 `Resolver`，两个模式都没有实现；② `Resolver` 非测试调用点 0；③ `agent.ModeOf`/`IsActive` 包外消费 0（唯一"消费"是同文件里 IsActive 调 ModeOf）；④ **最要命的一格**：`dto.AgentContext.AgentMode` 从来没被 `LoadContext` 填过 —— 实测三条读路（首次加载 / 缓存命中 / `agent_mode` 翻转后）拿回的都是空串。⇒ 就算有人接了 `Resolver` 也只能拿到空模式，"按模式分派"缺的第一块料是**模式读不出来**，不是分派代码。交付形状：`PassiveAgentLifecycle`（委托既有会话引擎 `HandleWithAgent`）与 `ActiveAgentLifecycle`（三个单方法依赖：`ConversationRunner` / `SOPExecutor` / `CustomerLookup`，真实构造分别是 `*service.SalesEngine`、`*service.SOPService`、`repository.CustomerRepository`，由编译期断言锁形状）；app 侧 `AgentLifecycleRuntime` **复用** `lifecycle.Resolver`（把项5 从 UNWIRED 翻成 wired，scope 收成 `internal/app`，理由：这一行盯的是"装配点还在用它"，不是"全仓某处调过"），外加分派三判据 —— 只有 `active`（先 `TrimSpace`+`ToLower` 归一，`  Active ` 与 `ACTIVE` 都是历史行可能长成的样子）且主动已装配才走 Active，**其余一切回退 Passive**（回退方向是有意的：被动只答一条消息，主动会对外群发，未知即主动等于运营在一个字段填错字就对外开火）；结果与 HTTP 响应都**回显实际走了哪条**（不回显时"真跑了主动"与"悄悄回退了"长得一模一样）。Active 的三步刻意各有一个"不做"：选材只认请求里那一个客户（群体选材在 T-P5-01 的圈选器，两处各圈一遍必然漂移）、决策只取智能体挂的第一个可解析 SOP（`DecisionStrategyIDs` 今天没有任何执行方读它，接进来是凭空造语义）、归因写进 `sop_executions.execution_data.one_id`（不加列、不加表）。C7 裁定的兑现方式：Active **不自建 Agent 循环**，整次运行交给 `SOPService.Execute`，并且本包**不许 import 会话引擎所在的包** —— 这条由 `TestActiveNeverImportsConversationEngine` 用 `go/parser` 读磁盘上的真实 import 集合钉住（不是编译期断言：那只能证明"这版没调"，证明不了"加 import 会被发现"），变异实测加一行 `_ "…/agent/runtime"` 即红、红因指到 `active.go`。**台账现值 55/64**：项5 转防回退登记，新增项19 六行 = 两行 wired（`InitAgentLifecycles` 启动装配点 / `SetupAgentLifecycleRoutes` 路由登记点）+ 四行按 UNWIRED 刻意留档（`ModeOf`、`IsActive`、`AutoExecute`、`DecisionStrategyIDs`）。那两行 wired 的**症状是跑出来的不是推出来的**：把 `router.go` 里那两跳各注释掉一行 ⇒ 门 rc=1 点名这两格，而 `internal/app` **整包 132s 全绿**（因为本卡用例是直接调 `SetupAgentLifecycleRoutes`/`NewAgentLifecycleRuntime` 的，它们证明不了 `router.Setup` 里还挂着）；第一格尤其要说清 —— 路由照挂、编译照过、线上每次运行稳定回 503，一个"永远没装配"的端点比没有端点更难发现，因为它看起来在工作（有响应、有 JSON、有 code）。**本卡之后仍不成立的部分**：① 渠道入站链路一行未动（仍走 `SmartCSOrchestrator` → `HandleWithAgent`），AC③ 靠的就是"被动只是多了一个可选入口"；② 新端点只带 `agent_id`/`customer_id`，没有渠道、账号、静默时段这些上下文，也不是给运营台用的成品（运营侧入口属前端竖）；③ **Active 这条路上的外联是否被发送前闸门拦住，本卡一条判据都没加** —— 编排走 SOP 只是把路收窄成一条，闸门在那条路上是否真的生效属 T-P5-03；④ `agent.ModeOf`/`IsActive`（运行期不读）、`SalesRequest.AutoExecute`（**写入 5 处、读取 0 处**，真正的开关是编排器里进程内的 `o.enableAutoReply` ⇒ 改这个字段不会开关自动回复）、`AgentContext.DecisionStrategyIDs`（`LoadContext` 连抄都没抄）四格仍按 UNWIRED 登记，接线前不得对外宣称可用 |
...
| G16 | `ops/service/conversion_funnel.go` `BuildFunnel` / `GetStageDetails` | **漏斗真实源的两处读数口径（T-P2-03 实测登记，本卡刻意不改——改它等于改现网看板上的数字，需要先定"宁可少给数据还是要吵"）**：① 四个 count 的错误**全部**被 `_` 吞掉 ⇒ 任一数据源表不可用（`intent_records` 由迁移建表、被禁止进 AutoMigrate 清单，新库上它可能压根不存在）时接口仍回 200，该阶段显示为 0，报表侧读成"转化率偏低"而不是"取数失败"；② 阶段之间无单调性约定 ⇒ `rate` 可 >100、`drop_rate` 可为负（`conversion_funnel_baseline_test.go` 的 `_NonMonotonicStagesBaseline` 按现状钉住，另有一条 `_UnknownStageIsBaseline` 钉住"未知阶段回 200 + 空名字 + 0 计数、不判 404"）。两处都属"数据可信度"问题 ⇒ 归口建议：任何拿这份漏斗做经营决策的卡（T-P4 漏斗收口、T-P8 指标看板）开工前先定口径，本卡只保证**改动前后逐字节相同**（差分实跑见任务清单 r20）。**（T-P4-06 已把商机接成第五段，2026-09-21 实跑登记，见 schema 文档 §4.16.5：① 的口径本卡动了半场 —— 现在是**五个** count，前四个仍按原样吞错不出声（那条现状刻意保留，改它等于改四个既有阶段的现网读数），**第五段（商机）是唯一把取数失败打进日志的一条**（`Warnf` + 按 0 上报），因为 T-P4-05 之后 `opportunities` 只有一个生产者，"0"在这一格独有两种来路（真没转出来 / 表没建或接缝没装配），而读的人会拿它做经营判断；这条告警是行为不是日志，由 `_OpportunityLegFailureIsWarned` 抓 stdout 守着，删掉任一条 `Warnf` 就红。② 的非单调现状**没改**，但 `_NonMonotonicStagesBaseline` 的取段方式改了：它原先按 `Stages[len-1]` 取"末段"，加第五段之后末段从会话变成商机（rate 400/drop −300 变 75/25），用例会**因为绿着而失去意义** ⇒ 现按阶段名取会话段，前端摘要区同病同治（`List.vue` 的 `stages[0]`/`stages[length-1]` 换成 `countByStage`）；三块 KPI 与服务端 `total`/`conversion` 仍停在 访问→会话，"端到端转化率要不要走到商机"新登记为本卡欠账。顺带一句：本卡那句「改动前后逐字节相同」在 T-P4-06 之后只约束**前四段**（黄金用例新增断言钉住 `total`/`conversion` 与演示表零写入），响应整体不再是同一串字节 —— 那是新增数据源，不是口径漂移 |
