...
- **已收 = Σ 计入结清的回款行**，所以它只能有一份。两处各存一份的后果与"报价头存合计"同一条 —— 一处加了新状态，另一处静默不算。计入哪些状态只有一个事实源 `model.PaymentStatusesCounted`（今天恰好是 `[confirmed]`，`model/payment.go:123`），而求和这件事全系统**只有一处**：`repository/payment.go` 的 `SumSettledByBill`，条件由那一格生成（本层不另写字面值）。service 层刻意**不把行搬进 Go 里再加一遍**（`service/payment.go:370` 那句注释就是这条判据的落地），它只拿那个和去做一次纯比较（`settlementOf`）。
...
- **`TZ=UTC` 复跑本卡全部六个包**（`-p 1 -count=1 -timeout 40m`，`01:50:07 → 02:05:50`，`rc=0`）：`pkg/db 72.851s`、`model 1.491s`、`repository 98.994s`、`service 555.636s`、`controller 174.708s`、`app 22.043s`。跑这一趟的理由不是"顺手再看一眼"：本仓有过一次时区裂脑（PG 会话钉在 CST，而 Go 侧按宿主机时区格式化日期 ⇒ 同一份代码只在 UTC 16:00–23:59 那段红），而 `bills`/`payments` 两边都有 `timestamptz`，`paid_at` 与"结清到哪一天"的比较正是这类雷的落点。宿主机时区是 `Asia/Shanghai`，所以这一趟测的是**另一条边**。
...
- 启动钩子的失败处置探针 `scripts/mut_startup_hook_p702.py`：**2 刀 2 杀**（`total=2 killed=2 alive=0 broken=0`，`01:40:49 → 01:40:55`），`migrate.go` 前后 md5 同为 `1607be56a9c91dee9716cda64290129a`（活树那一份；克隆里同一格是 `74b8efc1…`，见上一条）。两刀的红因分别落在**自己那一臂**上（① 摘掉 nil 守卫 ⇒ 报"① nil 句柄"；② 把 DROP 失败分支的 `return` 换成 `panic(err)` ⇒ 报"② 连接池已关"），不是同一条断言被顺手带红。锚点全部限定在钩子函数体区间内：`if db == nil {` 在 `migrate.go` 里有 5 处、`\n\t\treturn\n\t}\n\tconst ddl` 有 2 处，整文件替换会打到别人的分支上，那种"红"证的不是这一格。
- 上面这几笔测的是**提交之前的工作树**（HEAD `1bce38c3` ＋ 本卡未提交改动）。提交之后（`5b92525c`，已双推）另起一棵 `--shared` 克隆重跑一遍，那棵树里 **worktree 与 HEAD 逐字节相同、`git status --porcelain` 零行**，测的才是"已提交字节"：`go build ./...` 与 `go vet ./...` 各 rc=0 **且零输出字节**（vet 会编 `_test.go`，这一条才是"别人干净克隆里编译得过"的证据），`make audit` rc=0（10 段全过），`make fmt-check` rc=0，`golangci-lint 2.10.0` 与 CI 同版、`0 issues`，架构门 rc=0，台账门 rc=0 **71/81**（与上面那条"抽暂存区"的读数是两条独立通路得出同一个数）。
- 三组电池在同一棵克隆里整族重跑，结果与活树一致：读侧 **12/12**、启动钩子 **2/2**、台账 **5/5**，三串 `rc=0`、`ALL-CASES-KILLED-AND-SOURCE-RESTORED`，台账门还原后复跑仍 71/81 rc=0。**一处必须写下的差异**：`migrate.go` 那一格的 md5 基线两棵树不同（活树 `1607be56…`、克隆 `74b8efc1…`），差的正是并行会话那两处 `browsermodel` 表注册 —— 这两个 md5 **不该**相等，谁拿"两次跑的 md5 一样"当作"测的是同一份字节"的证据，在这一格上会拿错。而 `controller/bill.go` 那一格两边同为 `4db2da22…`（本卡之外没人碰它），这条相等才是同树证据。
- **上一张卡的电池被本卡打坏了六格，同轮修完并整族复跑**：`scripts/mut_bill_p701.py`（T-P7-01 的 71 格）在提交字节上（克隆 HEAD `ed07b978`，prepare 侧断言"本卡文件已在 HEAD 15/15、装配锚点 8 处各命中 1 次"）出 **4 处锚点落空 + 2 处存活**。落空的是 K10（跃迁表放宽后 `\tBillStatusVoided:  {},` 那格已不存在）、K39（接口从五方法长成六方法、用例随之改名 `…Five`→`…Six`）、K92/K93（挂载行从 `NewBillController(derive)` 变成 `(derive, read)`、日志行换词）；存活的是 G3（台账 23a 的探针只摘 `internal/app/bill_wiring.go` 一处 `WithDB`，而 23a 的判据是**目录级存在性** —— 本卡在 `payment_wiring.go` 里造了第二个消费方，摘掉第一个门照样绿）与 G4（23e 那格探的是"账单跃迁口有没有被派生腿顺手调用"，而本卡之后**真正的调用方出现了**在 `service/payment.go`，探针打的仍是派生腿那一侧，注掉它不改变任何读数）。修法不是把期望放宽：G3 改成一次注码跨两文件（`bill_wiring.go` + `payment_wiring.go` 各摘一行，为此驱动层加了 `cell_rels`/`apply_cell` 的多文件叠加，仍守"内存里叠完一次写盘"），G4 把刀口挪到**新增的那个真实调用方**上（正向探针：拆掉 `payment.go` 里那句 `s.bills.UpdateStatus(` ⇒ 门必须报 23e 从 wired 翻回 unwired）。复跑：**`KILLED=71 SURVIVED=0 RED-UNNAMED=0 BUILD-BROKEN=0 ENV-BROKEN=0 NO-RUN=0 格子数=71`**、`battery_rc=0`、"全部格子已还原（逐文件 md5 与基线一致）"，`battery_rc` 由脚本自己印，未经管道摘取。控制组**放刀前现测**为 app 3 / ctrl 24 / db 6 / model 9 / repo 11 / route 11 / svc 18 = **82 条**（T-P7-01 那趟是 66），多出的 16 条正是本卡在 `TestBill*` 前缀下的读侧用例 —— 这些 runner 按前缀收用例，隔壁卡加用例会让分母漂 ⇒ 写死计数的那套判据（"PASS+FAIL 必须等于控制组"）在这一族里只能现测，这条已在口径里，本条只是它第一次被真实撞上门。**一句话教训**：任何"目录级存在性锁"被写成"单消费方"假设的探针时，都会在第二个消费方落地那天变成一台永不开火的枪。
...
| v1.20 | 2026-09-23 | @backend | **T-P7-02 交付后的两处补记（§4.22「本卡实跑」新增四条 + 「中心事实」一处订正）**：① 把"求和只有一处"这句从推论改成可核引用 —— `model.PaymentStatusesCounted` 落在 `model/payment.go:123`，而求和这件事全系统只有 `repository/payment.go` 的 `SumSettledByBill` 一处，service 层"不把行搬进 Go 里再加一遍"的判据就写在该方法的调用处注释里（`service/payment.go:370`）。② **`TZ=UTC` 复跑本卡六个包**（`rc=0`）：宿主机时区是 `Asia/Shanghai`，`bills`/`payments` 两边都有 `timestamptz`，而本仓有过一次"PG 会话钉 CST、Go 按宿主机时区格式化日期"的裂脑 ⇒ 不跑 UTC 那一趟只测了一条边。③ **提交态 `--shared` 克隆重测已提交字节**（`5b92525c`）：`build`/`vet` 各 `rc=0` 且**零输出字节**、`make audit` 10 段全过、`golangci-lint`（与 CI 同版）`0 issues`、台账门 `71/81` 与提交前"抽暂存区"那条独立通路得出同一个数，三组电池（读侧 12/12、启动钩子 2/2、台账 5/5）在克隆里整族复跑结果一致；**并记下唯一一处两棵树 md5 不该相等的格** —— `migrate.go` 活树 `1607be56…` / 克隆 `74b8efc1…`，差的是并行会话那两处 `browsermodel` 表注册，拿"两次 md5 相同"当"同一份字节"的证据在这一格上会拿错。④ **上一张卡的电池被本卡打坏六格，同轮修完并整族复跑 = `KILLED=71 SURVIVED=0`（控制组现测 82 条，T-P7-01 那趟是 66）**：四处锚点落空（跃迁表放宽、接口六方法 + 用例改名、挂载行 `(derive)`→`(derive, read)`、日志换词）+ 两处存活（G3/G4）。两格存活是同一类错：**台账 23a / 23e 的探针按"单消费方"假设写，而该行的判据是目录级存在性** —— 本卡造出第二个消费方（`payment_wiring.go` 的 `WithDB`）与第一个真实调用方（`payment.go` 里的 `bills.UpdateStatus(`）之后，那两把刀砍下去读数不变。修法不是放宽期望而是把刀口对准新增的那一侧（G3 一次注码跨两文件、G4 改成正向探针：拆掉真实调用 ⇒ 门必须报 23e 翻回 unwired）。教训一句：**任何"目录级存在性锁"被写成单消费方假设的探针时，都会在第二个消费方落地那天变成一台永不开火的枪。** |
