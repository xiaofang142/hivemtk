...
- **本轮真正补上的一刀：488 份产物此前全部只在未跟踪集里。** 现测 `git ls-files docs/superpowers/specs/ledger/logs | wc -l` = **0**、`git ls-files -o` 同径 = **488** ⇒ §23.19 1-补 那句"0 份仍在库外"用的是"没被忽略"，而"入库"要 `git add`。本段连同新档（提交时数 **493 份／12 轮次／31 趟次目录**，`find -type f` 与 `ls -d */*/` 现算；含本节随后落的 `40-gitleaks-final.log` 与 `50-md-links.log` 两档，那两档之后本段再无产物动作。**但"收口"没收住**：下一段的推送前自查与推送后的三笔回读，又给 `CIwiring/20260923-174539/` 添了八档（60 形状闸首版／61 真值包含闸／62 放宽形状闸／63 推送前终档／64 gitleaks 历史面归属／65 拆灶（含一次假绿实录）／66 CI 回读／67 常驻闸的键值对调事故）⇒ 磁盘现数 **501 份／12 轮次／32 趟次目录**，非 `.log` 仍 **0** 份）一次性 `git add` 显式路径入库；非 `.log` 文件 `find … ! -name '*.log'` 现数 **0** 份；`CapAB/20260923-133217`（空目录，驱动在写第一档前停过）已 `rmdir`——**有档无判读＝没有证据**，与 §23.19 6-补 那条同判。**推后终态四向复测（tip `e1f9122d`）**：`git ls-files docs/superpowers/specs/ledger/logs` = **501**、`git ls-files -o` 同径 = **0**、`find -type f` = **501**、非 `.log` = **0**、白名单条目 `grep -c '^!docs.*logs/[^*]*/$' .gitignore` = **12** ＝ `ls -d */` = **12**⇒ 『可跟踪』与『已跟踪』这一笔正式对上账。
- 泄露面按规矩分三处数（落盘前后各扫一遍，gitleaks 放最后）：产物里 `password=` 一律是 `password=****…` 的 maskedDSN 形状，本轮 3 档新产物在写盘处过 `scripts/redact.py::scrub()`；`gitleaks detect --no-git --source docs/superpowers/specs/ledger/logs` 与对提交后的 `git grep HEAD` 两向读数记在终档 `CIwiring/172115/40-gitleaks-final.log`（半径要写明：`--no-git --source logs` 扫的是树内工作副本，不含 tip 提交面与已推历史，那两层各自另算，见 §23.16 第 4 段的三层口径；又因这份档自己就写在 `logs/` 里，扫描必须排在会产物的动作之后 ⇒ 终档读两向数：写档前一次、写档后一次，两向 scanned 若不等，差值只能是这份档本身）。

**7-补 · 那句"现断言 口令形状命中=0"是一处假绿，本段把它改回真读数（提交 `7bb28240` 之后、推送之前）。** 上一段原写的是"写盘处过 `scrub()` 并现断言 `口令形状命中=0（须 0）`"——那句断言当时是**写在 `echo` 里**的，`echo` 不判返回值，实测打印的是 `口令形状命中=9（须 0）`，也就是说**它自证了红却照样把提交做了**（[[feedback-cli-toolchain-gotchas]] "门禁缺产物退 0＝SKIP 不是 PASS"、[[feedback-verify-by-running]] "`grep -c` 是计数不是结论"的第三次兑现：这次连"断言"两个字都是装饰）。9 命中逐条读毕（`CIwiring/174539/60-password-shape-gate.log`）：全部是 `POSTGRES_TEST_PASSWORD=<hivemtk/.env 的 POSTGRES_PASSWORD>` 这一种**配方占位**，4＋4 行在 `CapAB/20260923-134129/A@8201-r1.log`／`B@8201-r1.log`（那两趟是打向 8201 的反向控制，键名大小写不一，`grep -i` 才捞得到），1 行在 `P702hook/20260923-153535/00-control.log`；非占位形状 `grep -icvE 'password=<[^>]*>'` = **0**。
- 但"9 枚全是占位"这句话本身仍然只是形状判断，不足以支撑"能推"。补一枚**真值包含闸**（`61-truesecret-containment.log`）：不问形状，直接取 hub `.env` 里 19 枚口令/密钥/令牌类值 ∪ 容器 `mtk-postgres` env 里 1 枚（按值去重后 **11 枚候选**，值长 8–65，`POSTGRES_PASSWORD` 在列）逐枚在四个面上数出现次数——树内 494 份 `.log`／`7bb28240` 全量差异／审计文档／探针脚本，另加第五面"整棵 tip 树 `git grep`"。读数：**私有真值出现合计=0**，`v5 rc=0`；反向自证（把候选里第一枚临时拼进一行假日志）未种=0／种入=1 ⇒ 闸有牙。唯一非零那枚是值长 8 的 `HIVEMTK_ADMIN_PASS`，闸把它单列为**公开常量**：它本身就是文档化的默认弱口令串，已跟踪源码 20 处命中（`user-server/internal/service/password_policy.go` 的禁用词表、`auth_test.go` 用例名、`scripts/bootstrap.sh`），本趟产物里的 4 处是 `go test` 的**子用例名** `TestValidatePassword_ForbidCommon/<该串>`（`CapAB/134337/full-A@8234-r1.log:9818,9824`、`full-B@8234-r1.log:9844,9850`）——被"禁止"的那个词，不是被使用的那个词。存量实例的 admin 口令是否仍是这个默认值＝另一件已登记在案的事（install/seed 三重锁 ＋ `scripts/rotate-admin-password.sh`），本闸不动它。
- 顺带把形状闸放宽到 `token=`／`secret=`（`62-shape-v2-token-widened.log`，**rc=1 是设计如此**，不是新红）：23 枚 RAW 全部读毕 ⇒ 计数值（`token=1234` 类）、合成夹具串（`security-token=` 后那枚是测试自己造的 `*-LEAK-J` 哨兵，正为"落进错误串就该被点名"而写）、文档正文里 `` `password=` `` 后面紧跟的那个反引号（值长=1）；无一是真值。这一放宽版**不当门禁**用：它的判据（形状）比上一段那个包含闸弱，留着只作逐条判读的底档。
- 由此定下本泳道的新规矩：**"口令形状命中"这类断言要么写成真会 `exit 1` 的脚本，要么不许写在 `echo` 里**；推送前以包含闸为准，形状闸只算初筛。未覆盖面第 8 段①–⑧ 号之外新增⑨：**包含闸只覆盖这台机器当前取得到的那 11 枚候选**（`.env` 与容器 env 的并集，键名须命中 `PASS|SECRET|TOKEN|KEY|DSN|CRED|SALT`）——历史里别的机器上、别的键名下的真值，它看不见；那层面仍只有 gitleaks 的规则集在守，而 gitleaks 的 `--no-git` 半径见上一段（此条已同时列作第 8 段⑨）。
- 推送事实（`64`／`66` 两档）：闸绿之后按授权推双远端，`f4da0192..2c447c79`（本批共四笔：`f8d83b59`、`2ab6a4b9`、`7bb28240`、`2c447c79`），GitHub 与 Gitee 的 `refs/heads/master` 回读**同为 `2c447c79802c4e25495a9961c1be089224842eaa`**＝本地 HEAD；推前先做祖先判定（`git merge-base --is-ancestor f4da0192 HEAD` ⇒ 两推皆 fast-forward，无 `--force`）。推前另补一刀 gitleaks **历史面**归属（`64` 档）：`gitleaks detect --source .` 全史 1236 commits／65.27 MB ⇒ 1883 枚 findings、涉 26 笔提交，但**本批四笔逐笔 0 枚**（那 1825 枚来自别人历史里的 `interactions.jsonl`）——CI 的 `gitleaks-action@v2` 在 push 事件上判本次推送区间，故这批产物不会把它现有的绿翻红。
- 推送后 CI 回读（`66` 档，tip `2c447c79` 对上一 tip `f4da0192` 做差集）：`Markdown Lint`／`Docs Consistency`／`Docs Link Check`／`SBOM` **四门 success**（497→500 份取证档与新文档段进树没弄红文档面）；`Lint` 仍 failure，两枚 error **逐字与上一 tip 相同**（`216:7 preserve-caught-error` 在 `user-web/browser_automation/src/core/cdp/input.js`、`821:15 no-useless-assignment` 在 `.../core/primitives.js`，两文件最近改动是 `32d1bce4`／`c7f8f37e`）⇒ 浏览器自动化泳道的**已提交**字节，第五十二轮那句"属未提交面"就此订正为"公开既有红"；`user-server-ci` 与 `Seam Guard` 本批**根本没跑**——两份 `paths:` 是显式文件清单（`user-server-ci.yml:9-23`、`seam-guard.yml:25-32`），本批动的 `docs/**` 与 `scripts/cap-ab-53300-probe.py` 一枚都不在里面 ⇒ 常驻探针自身也无 CI 执行点，并入未覆盖面⑦那一类。**再往后的两推把这条触发规则量实了**（`66` 档后两段）：只改 `docs/**` 下的 `.log` 时，CI 上**只有** `Lint`（`lint.yml:15-16` 的 `push:` 无 `paths:`＝每次推送恒跑）＋`Docs Consistency`（`docs-consistency.yml:23-24` 含 `'docs/**'` ⇒ **取证档本身受文档一致性门守护**）＋`SBOM` 三趟，`Markdown Lint`／`Docs Link Check` 不跑（两者 push `paths:` 首条皆 `'**/*.md'`，`markdownlint.yml:29-33`、`docs-link-check.yml:21-26`）⇒ 本泳道改 `.md` 与改 `.log` 的 CI 面不是一回事，判"文档门绿"要先说清动了哪类字节。
- 拆灶（`65` 档，含一次**当场抓住的假绿**）：`CLONES="a b c"; rm -rf $CLONES` 在 zsh 下不拆词 ⇒ 删除命令拿到一个"路径里带空格"的不存在目录、`-f` 让它静默退 0，复测循环同样只迭代一次、把九个路径当一个判 `-e`，于是档里第一版打印出 `GONE …九个路径同一行`——**九份一份没删**。改为一行一个路径重删＋逐条 `ls -d` 复测（9 行各自 `No such file or directory`）才真清掉；同一条链里另一处 zsh 坑同时开火：`rm -f /tmp/r45-*.out …` 中一个 glob 无匹配 ⇒ zsh 默认整条命令 `no matches found` 中止，一串手稿一个没删，补 `setopt NULL_GLOB` 后重跑。`docker rm -f r45-pg100` 一次成功（复测 `docker ps -a --filter name=r45-pg100 -q | wc -l` = 0）、采样器残留进程拆前拆后皆 0、`/tmp/gocache-r45mut`（3.2G）**有意保留**——它是 `scripts/mut_egress_pool_r30.py:525` 的默认私有 GOCACHE。

**7-补2 · 已推出去之后，把那枚包含闸做成了常驻件——升级过程本身交了一次学费，也是一次"假绿长在自己写的闸里"的实录。** 一次性 `/tmp` 脚本不算门禁（下一位无从复跑），故落 `scripts/check-secret-containment.py`：候选＝hub `.env` 里键名命中 `PASS|SECRET|TOKEN|KEY|DSN|CRED|SALT` 且值长≥6 的枚值 ∪ `docker inspect mtk-postgres` env，按**值**去重；五个面（树内产物／本笔差异／审计文档／探针脚本／整棵 tip 树 `git grep`）逐个 `count`；反向自证＝临时把一枚候选拼进一行假日志、断言 `planted == base + 1`。改写时把并枚写成 `{v: k}` 再 `for k, v in dedup.items()` ⇒ **键与值对调**：屏上打出 `键 dce21ad1da36…（值长=17）`（"键名"位是一枚真值，"值长"位是 `len("POSTGRES_PASSWORD")`），三处后果互相掩护——计数全打在键名上（键名当然满仓库都是）⇒ 11 枚一律被单列成"公开常量"、**`私有真值出现合计=0` 这句话当时是空的**（闸从没比过真值），反向自证还照样报"有牙"（种进去的也是那枚键名串，+1 成立）。坏版读数 `未种=22 种入=23`／`公开常量合计=1211`；好版 `未种=0 种入=1`／`公开常量 1 栏、合计=28`——**是这两行读数的差异抓住了它，不是通读代码**（[[feedback-verify-secondhand-review-claims]]：自己刚写的东西同样是二手证据，跨版本要对数字）。修法＝按值并枚、一律以 `(键名, 值)` 迭代，并在**任何打印之前**加形状守卫 `re.fullmatch(r"[A-Za-z_][A-Za-z0-9_]*", k)`，不成立即 `return 2` 并打印"疑似键/值对调，禁止输出"；守卫按反向测验（把 `env_values` 替成一枚非键名形状的假值跑 `main()`）⇒ 实测 `rc=2（须 2）`、"输出里是否出现那枚假值"= `False`。好版全量复测：`候选真值枚数=11 … 私有真值出现合计=0（须 0）… rc=0`，`gitleaks detect --no-git --source …/logs` `no leaks found`，`check-md-links-offline.py` `断链 0 处`。**两处半径必须同时记下**：① 本档写之前最后一次好版跑是"树内 500 份"，本档之后磁盘 **501 份／12 轮次／32 趟次目录**（非 `.log` 仍 0）⇒ 这条闸的读数永远不含它正要写下的那一份；② **坏版那一跑把 11 枚候选值打进了会话输出**——仓库、取证档、tip 树、双远端均无（好版复测那行 0 就是证据），但它们留在了本机的会话转写里；转写按整份处置、不能改单行（那是伪造取证链），已并入下面⑨。**它没有注册进 `make audit`，是有意为之**：候选源在本机（`.env` ＋活容器），别人机器与 CI 上取不到同一批值 ⇒ 注册只会得到一扇恒 `rc=2` 的哑门（rc=2 是"没跑过"，[[feedback-verify-by-running]]）；CI 侧这一维仍只有 gitleaks 的规则集在守。全部产物在 `CIwiring/20260923-174539/67-resident-gate-swapbug.log`。
