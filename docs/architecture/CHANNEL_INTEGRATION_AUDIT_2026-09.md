...
- **本轮真正补上的一刀：488 份产物此前全部只在未跟踪集里。** 现测 `git ls-files docs/superpowers/specs/ledger/logs | wc -l` = **0**、`git ls-files -o` 同径 = **488** ⇒ §23.19 1-补 那句"0 份仍在库外"用的是"没被忽略"，而"入库"要 `git add`。本段连同新档（提交时数 **493 份／12 轮次／31 趟次目录**，`find -type f` 与 `ls -d */*/` 现算；含本节随后落的 `40-gitleaks-final.log` 与 `50-md-links.log` 两档，那两档之后本段再无产物动作。**但"收口"没收住**：下一段的推送前自查又添了 `CIwiring/20260923-174539/` 四档（60 形状闸首版／61 真值包含闸／62 放宽形状闸／63 终档）⇒ 磁盘现数 **497 份／12 轮次／32 趟次目录**，非 `.log` 仍 **0** 份）一次性 `git add` 显式路径入库；非 `.log` 文件 `find … ! -name '*.log'` 现数 **0** 份；`CapAB/20260923-133217`（空目录，驱动在写第一档前停过）已 `rmdir`——**有档无判读＝没有证据**，与 §23.19 6-补 那条同判。
