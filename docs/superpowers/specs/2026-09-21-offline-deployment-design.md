...
⇒ **订正（2026-09-23，见 §7.5）**：这笔对账把差额**整笔**归给了"索引里有、盘上已删"，漏了另一半：
两条枚举源对同一文件写法不同（`x` 与 `./x`），去重发生在剥 `./` **之前** ⇒ 同时被两条源枚举的 `.env` 形状文件
各进清单两次，`scanned` 因此**虚高**。今天在同一刻量 platform 树：raw 270 / 唯一 266（折掉 4 个双份）/
白名单在枚举里 2 / 盘上缺失 **0** ⇒ 老闸印 270−2=**268**、新闸印 266−2=**264**。
即当时那个 268 里"已删"与"双份"两件事混在一格，其中双份占 4（今天该树的删除项已清零，差额只剩双份），
所以那两句"= 268 ✓ 自洽"只做到**数字对得上**、没做到**归因对得上**。
...
## 7.5 第五轮：闸自己的 `scanned` 数是虚高的（2026-09-23，指令 = 不留下任何问题）

起因是复核交付台账里那一句"克隆侧 `scanned=4242` ≈ 该 SHA 的 `git ls-files` 4243，差 1 = 索引里有、树里已删的那类"。
在"只含已提交内容"的 `--shared` 克隆里实测：盘上缺失的候选 **0 个**、白名单命中 6 条、唯一候选 4243
⇒ 那句"差 1"根本不成立。真正的差额来自枚举口径：**`scanned` 把同一文件数了两次**。

- 机制：① 源（`git ls-files`）给 `user-web/.env.example`，② 源（`find`，扫 gitignored 的 `.env` 形状文件）给
  `./user-web/.env.example`，两副面孔字符串不同，而老写法在 `candidates()` 里就 `sort -z -u` —— **去重发生在剥 `./` 之前**，
  折不掉；循环里才 `f="${f#./}"`，于是同一文件在最终清单里各留一条。
- 规模（同一刻实测，测于 hivemtk `a9c2aea6` / platform `82a7c3f`）：hivemtk 5 个、platform 4 个文件双份，
  全部是 `.env` 形状文件（hivemtk：`scripts/inference-host/models.env`、`user-server/.env.example`、
  `user-web/.env.example`、`user-web/.env.development`、`user-web/.env.production`；
  platform：`.env.example`、`platform-contributor/.env.example`、`platform-web/.env.development`、`platform-web/.env.production`）。
- 危害不在"数字难看"：最终清单同时是喂给 `xargs grep` 的对象 ⇒ **这些文件里的一处命中会被 grep 两遍**，
  印两行同样的 `file:line`、`FAIL … N hit(s)` 计数翻倍。而这 9 个文件正是"示例基址"最可能回落到旧域的那一类
  （`.env.example` 就是给人抄的），也就是本闸最该数准的地方。

**修法（两仓各一份，逐字同构）**：`candidates()` 不再预先去重，循环写进 `TMP_LIST_RAW`，
归一化之后 `sort -z -u < RAW > TMP_LIST`，`SCAN_FILES` 改为数最终清单里的 NUL 字节 ⇒ 自证数与待扫集合同源，
不再靠循环里另起一个计数器（计数器与清单本来就是两份真相，这正是它能长期漂移的原因）。

**验证（全部真跑，两棵树各一整套）**

| 腿 | 读数 |
| --- | --- |
| 同一刻"独立复算 vs 闸自报"（hivemtk 活树） | raw 4337 / 唯一 4332 / 缺失 0 / 白名单 6 ⇒ 期望 **4326**；修后闸印 `scanned=4326`（修前同刻印 4331） |
| 独立复算 vs 闸自报（platform 活树） | raw 270 / 唯一 266 / 缺失 0 / 白名单 2 ⇒ 期望 **264**；修后 264（修前 268） |
| 克隆树（`--shared` @ `98470a82`，只含已提交内容） | 期望 4237 = 唯一 4243 − 白名单 6；修后 4237（修前 4242） |
| 反向 A/B：往**双份清单里的那个文件**注入 1 行旧域 | hivemtk（`user-web/.env.example:30`）：老闸 `2 hit(s)` 且同一行印两遍 / 新闸 `1 hit(s)` 一行；platform（`.env.example:55`）：老 2 / 新 1 |
| 两仓自带反向电池 `check-no-xapptool.test.sh` | 各 rc=0，三腿 PASS（ASCII 夹具 0→1、中文名夹具 0→1、撤夹具回 0） |
| 还原 | 夹具两份均 `cp` 备份 + `md5` 全等（`61a01f0a…`、`718ffe0e…`），撤夹具后新闸回基线；platform `git status --porcelain` 只余闸自身那 1 个 ` M` |
| 邻门 | `check-shell-cjk-expansion.sh` rc=0（命中 2 = 基线 2，无新增形状）；`bash -n` 两仓各 0 |

**为什么这道缺陷能活到今天**：仓库基线早已归零 ⇒ 双份枚举只表现为"scanned 多 5"，不改变 rc；
而 `scanned=N` 自证的设计目的一直是"证明闸真扫了东西"，虚高的数同样能证这件事。
它第一次咬人是在**拿这个数对账**的时候（§Task 21 那次把 268 的差额整笔记成"在途删除"，见该节订正）。
判据补一条：**凡把门的自证数当账用，先独立复算一遍"唯一集 − 白名单 − 缺失"**，别用减法倒推差额归给谁。

两仓副本的同步面复核：修完后 `diff` 两仓闸，非注释差异仍只有白名单那 4 行（platform 不需要那 4 条形不成例外），
执行逻辑逐字一致。

### 7.5.1 上一表"反向电池三腿 PASS"没有抓住这道缺陷 —— 补第四条腿（2026-09-23）

上面那一行"两仓自带反向电池 … 三腿 PASS"是**修前修的**读数，它本身就是本次要记的反例：
三条腿全绿的同时，双份枚举缺陷就在场。原因在注码落点——①/①b 打在**仓根新建的未追踪 `.md`** 上，
那种文件只有 ① 源枚举得到（`find` 只认 `.env` 形状），② 源根本不参与 ⇒ 去重时机对不对，三条腿一条都量不到。
**"门有牙"的证据只覆盖注码打的那个面**，这跟 §7.5 主线的口径是同一件事的另一面。

补的第四条腿（`check-no-xapptool.test.sh` ④，两仓逐字同一份）：
- `pick_dual()` 从 ② 源的输出里挑**同时被 ① 源枚举到、且不在白名单**的那个文件（即双源交集），
  没有则印 `SKIP ④：…（不是测过且通过）` 而不是静默当通过；
- 往该文件末尾追加 1 行旧域 → 断言 `FAIL … N hit(s)` 恰为基线 +1，**且**用 `count_prefix()`
  数该文件在红因里被点名的次数必须 = 1（`.env.example` 是 `user-web/.env.example` 的子串，
  所以按"行首 = 路径 + 冒号"匹配，不用包含匹配）；
- 这条腿改的是仓库已有文件 ⇒ 还原走 `cp` 备份 + `trap`（含 `set -e` 中途死）+ md5 全等校验，再跑一遍闸确认回基线。

**反向测试自己的反向测试（先编后改，两棵树各一次）**：把两仓的闸临时换回修前那版
（`git show 6d11065f^:…` / `git show 0552a40^:…`，`cp` 覆盖、跑完按 md5 还原），第四条腿必须红——
实测 hivemtk 印 `BROKEN: 往 user-server/.env.example 注入 1 行后命中数应为 1，实为 2`，
platform 印同型句（`platform-contributor/.env.example`，应为 1 实为 2），两树 rc 均 1；
换回修后闸复跑两树各 rc=0、四腿全 PASS。`SKIP ④` 分支也用探针实测过：
在 `/tmp` 现建一个 `.env.example` 被 `.gitignore` 挡住的探针仓（只有 ① 源摸不到它 ⇒ 交集为空）跑一遍，
印出 SKIP 句且 rc=0，探针目录已删。

测于 hivemtk `d6be6d31` / platform `0552a40` 的同一刻读数：hivemtk `scanned=4330 / 0 hits`，
...
**但"本地 16/16"不等于"CI 16/16"——首跑就是在 CI 红了一格，红因是夹具而不是判据。**
`3bb24cba` 推上去后 run `35816241272`：`check-shellcheck` 步骤 success，`反向测试` 步骤 **failure，
PASS=15 FAIL=1**，那一条打在 T4：`line 179: bash: command not found`、rc=127（期望 2）。
读出来的根因是**两端的 PATH 形状不同**：runner 镜像在 `/usr/bin` 与 `/bin` 里都自带那个工具，
"把装着它的 PATH 目录摘掉"就把 bash/python3/git/dirname 一起摘了，于是量到"环境坏了"而不是"缺工具"；
本机全绿是因为本机它只装在 `~/.local/bin`，摘掉那一个目录不伤别的。修法三条，缺一不可：
① 摘完之后**垫**一个腿内目录（放 bash/git/dirname 的绝对路径软链），让"只缺那件工具"这个前置在两端同形；
② 腿内的 python3 取 `sys.executable` 的**真解释器**——本机 `command -v python3` 是 pyenv 的 shim，
shim 自己要 PATH 上的帮手，把 shim 放进去会把闸卡在"起解释器"（实测挂 40s 不起，既不红也不绿）；
③ 一律用**绝对路径**调 bash，因为 `PATH=<新值> bash …` 里"新值是否参与 bash 自身的查找"在本机
...
2. CJK 门：本机 `命中 2 ＝ 基线 2`（绿、无提示），CI `命中 1 处（基线 2 处）` 外加一条
   ...
   这一点不必信 CI —— 在 `git clone --shared` 出来的**只含已提交内容**的树里复跑同一道门，
   读数逐字复现 CI：`scanned=137 个 shell 文件，命中 1 处（基线 2 处）` ＋ 同一条 STALE 提示，rc=0。
