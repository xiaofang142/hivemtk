...
## 🏆 规则0b：禁止使用子 Agent 并行（RPM 限制，最高优先级）
- 因 RPM（请求速率）限制，**禁止调用子 Agent（Task/搜索子代理等）并行执行任务**
- 所有任务由主 Agent 直接执行，串行推进，不依赖子 Agent 分担
- 需要探索/搜索/实现时，直接使用主 Agent 的工具（Read/Grep/Glob/LS/WebSearch 等）
- 本规则优先于其他一切规则，与其他规则冲突时以本规则为准
