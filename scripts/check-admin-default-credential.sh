...
# =============================================================
# 反向测试（改完本脚本必须逐格真跑；最近一次实测 69aa2c60 之后 15/15 按判据收口）
#
...
# 夹具两个自身死法（本轮踩过，都靠下面标注的对照格兜住）：
#   · 后台起的服务不能用 `stdin.readline()` 续命（stdin 是 /dev/null ⇒ 立刻 EOF ⇒ 端口写了但没人监听）；
#   · PROBE_BASE_URL 必须 export 给探针，漏了这一格会直接打到默认档 127.0.0.1:8204 真实例上攒爆破计数。
#
...
#       （变异要断言真的改动了源文本，否则等于没变异；改名别只动前缀 ⇒ 锚点子串还在就是无效变异）
...
#   R8 只换 WEB_SETUP_SRC（把一项 e2e 字面量改成变异值、白名单填该变异值）
#      期望 真 spec rc=0 / 变异 spec rc=1 —— 红点跟着源走 ⇒ 候选不是写死在脚本里的第二副本
#   R9 --list 计数 == 独立口径（seed + 数组字面量去重，另用一套正则现抽）算出的项数
#      ⇒ 这一格专治"数组收尾行 `].filter(…)` 的尾巴被当成候选"：本批实测抽出来过假候选 `string`
#      （一次真失败登录），修法是只取数组项、剔 `//` 注释行。计数对不上就是又串味了。
# R2/R3b/R8 若不红、R4/R4b/R5/R6/R7 若不 rc=2、R9 若计数不一致 ⇒ 判据没牙，别提交。
