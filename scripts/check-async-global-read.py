...
口径边界（按「门禁口径盲区」的规矩写明，别把"没数到"印成"没问题"）：
  ...
  - **只数体内直读**：体内调用的那个函数**它自己**再读测试可改写的全局，属本门的**已知盲区**，实测四处
    （`FetchQQAttachment` 读 `qqAttachmentURLGuard`、`FetchTelegramMedia` 读 `tgAPIBaseOverride`、
    `FetchDingTalkRobotMedia` 读 `dingtalkOpenAPIBase`、抖音取址 helper 读 `dyAPIBaseOverride`）。
    这四全局只出现在各家 seam 的**默认实现**里——用例一装替身就走不到，本轮 CI 全量 -race 也没从这里出过红。
    本不当它存在，但**要把调用图收紧到一层传递得先改这些 Fetch 的签名**，而 `FetchTelegramMedia` 的三参签名
    被 `webhook_batchf4_m01_telegram_test.go:770` 一行 `var f func(context.Context, string, string) (...) = FetchTelegramMedia`
    钉成契约 ⇒ 不擅动别人的契约，此处按盲区写明；
