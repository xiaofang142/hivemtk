...
#
# 维护注记：注释里「# + 任意个空格 + 那个工具名 + 非指令文本」会被当成指令去解析，报
# SC1073/SC1072——**缩进也一样**，不是只有顶格才中招。本仓已三次砍到自己人：
# 本门头注释第 12 行、下面那句 用法 的续行、以及 .test.sh 里一句 `#   <工具名> 只装在 …`
# 的缩进续行。别的 shell 文件写注释时同理：那句话别从那件工具的名字开头。
#
