...
func (s *QuoteSendService) OpenApproval(ctx context.Context, quoteRowID string) (*model.ApprovalRequest, error) {
	...
	appr, err := s.open.GetPendingBySubject(ctx, QuoteApprovalSubjectType, id, QuoteSendPolicyKey)
	if err != nil {
		return nil, fmt.Errorf("quote send: 查 %s 的开放审批失败：%w", id, err)
	}
	return appr, nil
}
...
func (s *QuoteSendService) Send(ctx context.Context, in QuoteSendInput) (*QuoteSendResult, error) {
	...
	appr, err := s.verdict(ctx, row.ID, in.ApprovalID)
	if err != nil {
		return nil, err
	}
	switch appr.Status {
	case model.ApprovalStatusPending:
		if approvalWindowClosed(appr.ExpiresAt, s.now()) {
			...
			return &QuoteSendResult{
				Disposition: QuoteSendExpired, ApprovalID: appr.ID, Status: row.Status,
			}, nil
		}
		return &QuoteSendResult{
			Disposition: QuoteSendAwaiting, ApprovalID: appr.ID, Status: row.Status,
		}, nil
	case model.ApprovalStatusRejected:
		// 库里那一版**留**在草稿：报价生命周期的 rejected 说的是"客户拒了这份报价"，
		// 不是"内部审批没通过"。两件事合成一个值之后，"被自己人拦下"的报价就再也发不出去了。
		return &QuoteSendResult{
			Disposition: QuoteSendRejected, ApprovalID: appr.ID, Status: row.Status,
		}, nil
	...
	case model.ApprovalStatusApproved:
		return s.dispatch(ctx, row, appr.ID, in)
	default:
...
// verdict 取得本次要依据的那条结论：带着号来就按号读并核对身份，没带号就入队一条。
//
// **不**在这里猜"上次是不是批过"：审批侧没有"按 subject 读最新已裁决"那一格读口，
// 补一格的代价是让任何一次没带号的调用都能沿用旧结论（同一版可以反复发出去）。
// 少带号的后果因此只会是多一条待办 —— 更严，不会更松。
func (s *QuoteSendService) verdict(ctx context.Context, rowID, approvalID string) (*model.ApprovalRequest, error) {
	if approvalID == "" {
		appr, _, err := s.approvals.Submit(ctx, ApprovalSubmitInput{
			SubjectType: QuoteApprovalSubjectType, SubjectID: rowID, PolicyKey: QuoteSendPolicyKey,
		})
		if err != nil {
			return nil, fmt.Errorf("quote send: 为版本 %s 入队审批失败：%w", rowID, err)
		}
		if appr == nil {
			// Submit 的 (nil, false, nil) 不存在（要么给行要么给错），走到这里就是实现漂了：
			// 按"没拿到批准"处置，不发。
			return nil, fmt.Errorf("%w: Submit 对版本 %s 返回了空记录 ⇒ 本次不外发", ErrQuoteSendVerdictUnknown, rowID)
		}
		return appr, nil
	}
	...
	if appr.SubjectType != QuoteApprovalSubjectType || appr.SubjectID != rowID || appr.PolicyKey != QuoteSendPolicyKey {
		return nil, fmt.Errorf("%w: 审批 %s 是 (subject_type=%s, subject_id=%s, policy_key=%s) 的结论，"+
			"而本次要发的是 (%s, %s, %s) ⇒ 三列必须逐一对上，少比一列就是给别的对象开门",
			ErrQuoteSendApprovalMismatch, appr.ID,
			appr.SubjectType, appr.SubjectID, appr.PolicyKey,
			QuoteApprovalSubjectType, rowID, QuoteSendPolicyKey)
	}
	return appr, nil
}

// dispatch 已批准那一档的唯一出路：先补齐完整性判据（收件人、明细、话术），
// 再认领，再出域，最后回写事件。
func (s *QuoteSendService) dispatch(ctx context.Context, row *model.Quote, approvalID string, in QuoteSendInput) (*QuoteSendResult, error) {
	...
	if len(lines) == 0 {
		return nil, fmt.Errorf("%w: 版本行 %s 在库里而明细一条都没有（合计会算成 0.00 发给客户）"+
			"⇒ 这是 T-P6-02 点名的那条数据事故，须人工处置，本次不认领、不外发", ErrQuoteSendLinesMissing, row.ID)
	}
