...
// TestOrderWebhookAndCustomerLookup_FT 端到端功能测试：
...
func TestOrderWebhookAndCustomerLookup_FT(t *testing.T) {
	...
	if _, err := service.UpsertOrderFromWebhook(ctx, "taobao", "FT20260723001", "已付款", map[string]any{
		"order_no":     "NO-FT-001",
		"user_phone":   phone,
		"user_name":    name,
	...
	if _, err := service.UpsertOrderFromWebhook(ctx, "taobao", "FT20260723001", "已完成", map[string]any{
		"user_phone": phone,
		"user_name":  name,
	}); err != nil {
